...
noting that cell vector data is not present in an XYZ file.
"""
function readXYZ(filename::AbstractString; warn=false, err=stderr)
    open(readXYZ, filename)
end

"""
    writeXYZ(xtal::CrystalStructure{3} filename::AbstractString)

Writes 3D crystal data to an XYZ file.
"""
function writeXYZ(xtal::CrystalStructure{3}, filename::AbstractString)
...
function writeXSF(
    ...
    tostr(x::Number) = rpad(lpad(@sprintf("%f", x), 20), 22)
    open(filename) do io
        println(" DIM-GROUP\n 3 1\n PRIMVEC")
    end
    return nothing
